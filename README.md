...
>  어덕편덕은 이런 이유에서 이러한 방식으로 만들어졌어요!
 ...
 >"하... 그날 아이린 음방 짤 너무 아름다워서 나도 모르게 32894729857번 저장했다... " - 레드벨벳 덕후 대학생 OO씨<br>*
  ...
  <br><br>웹 서핑을 하면서 좋아하는 연예인 사진을 저장하다 보면 일관적이지 못한 파일 이름과 저장경로 때문에 찾아 헤매는 일이 자주 있었습니다.<br><br>
...
### 문제 해결 과정 
  ...
  4. 움짤 파일과 초고화질 이미지의 처리 <br>
  Clova CFR API 호출시 2MB 이상의 이미지는 처리가 불가능한 문제가 있었습니다. 대부분의 GIF 파일은 2MB를 훌쩍 넘었고 일부 초고화질 이미지 또한 처리가 어려운 문제가 있었으나, 저 둘은 덕후에게 결코 포기하기 어려운 존재라고 판단했습니다. GIF의 특성상 첫프레임 밖에 처리할 수 없는 제약은 있지만 이미지를 서버단에서 PNG로 변환한 다음 API를 호출 했고, 그 외 확장자의 이미지가 2MB가 넘는 경우에는 이미지의 크기를 축소하여 용량을 줄인 뒤 API를 호출했습니다.
...
* API 호출에 시간이 다소 소요되는 점이 다소 답답하게 느껴질 수도 있습니다. 가능한 속도를 개선하기 위해 노력했으나. 현재 5초 이하까지 시간을 단축하기에는 어려움이 있습니다. 이를 좀 더 개선할 수 있는 방법이 있다면 적극 검토하겠습니다!
